...
<Tabs>
  <TabItem value="CentOS/RHEL" label="CentOS/RHEL" default>
    ...
    * Your user account has permission to run `sudo`.
    ...
    Obtain the install script's SHA-256 digest from an official OceanBase release channel independently of the script download URL. Then download, verify, review, and execute it:

    ```shell
    : "${SEEKDB_INSTALL_SHA256:?Set this to the officially published SHA-256}"
    SEEKDB_INSTALL_SCRIPT="$(mktemp)"
    trap 'rm -f "$SEEKDB_INSTALL_SCRIPT"' EXIT
    curl --proto '=https' --tlsv1.2 -fL \
      https://obportal.s3.ap-southeast-1.amazonaws.com/download-center/opensource/seekdb/seekdb_install.sh \
    ...
    Do not continue if no official digest is available or checksum verification fails. Prefer offline installation for a version-pinned deployment.
  ...
  <TabItem value="Debian/Ubuntu" label="Debian/Ubuntu">
    ...
    * Your user account has permission to run `sudo`.
    ...
    Do not use an APT trust override; it disables repository signature verification. Until the official SeekDB documentation provides a signing-key URL and a `signed-by=` repository configuration, use the offline package workflow below. Do not invent or retrieve a signing key from an unofficial keyserver.

    #### Offline installation

    1. Download the seekdb package for your desired version from the [seekdb Download Center](https://www.oceanbase.ai/download). Obtain its SHA-256 digest from an official OceanBase release channel independently of the package URL, and copy the package to the target machine.

    2. Verify the package before using `sudo`:

       ```shell
       : "${SEEKDB_DEB_SHA256:?Set this to the officially published SHA-256}"
       : "${SEEKDB_DEB_FILE:?Set this to the exact downloaded .deb filename}"
       printf '%s  %s\n' "$SEEKDB_DEB_SHA256" "$SEEKDB_DEB_FILE" | sha256sum --check --strict
       ```

       Stop if no official digest is available or verification fails.

    3. In the directory containing the verified package, install seekdb with dpkg:
