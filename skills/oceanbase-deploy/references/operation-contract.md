...
Before an OBD command that can submit telemetry, record the installed build, current telemetry submission and telemetry-log settings, applicable organizational policy, expected event categories, network destination when established, local log path, and whether command errors or parameters can enter the report or a child-process argument. The V4.6.0 command guide documents telemetry as enabled by default for `cluster autodeploy`, `deploy`, `start`, `stop`, `reload`, and `upgrade`; verify the selected installed build rather than assuming this list is exhaustive or unchanged.

Treat telemetry command construction as a fail-closed version gate, separate from telemetry policy. In the reviewed `ob-deploy` snapshot `4ef23088...`, the parent process JSON-encodes an error buffer, interpolates that JSON inside a single-quoted command string, and launches it with `shell=True`. JSON encoding is not shell quoting: an apostrophe or other shell syntax in error text can escape the intended argument. Before any command whose installed path can dispatch that background task, prove that the selected build passes both deployment name and telemetry data through an argument vector without a shell, or uses an equivalently reviewed transport that remains safe for arbitrary error text. If that proof is absent, treat ordinary plugin, remote-host, path, SQL, and command error text as untrusted and unbounded and stop before the command. Do this even when submission is configured off: `TELEMETRY_MODE=0`, `TELEMETRY_LOG_MODE=0`, or an outer shell quote is not containment because the parent shell parses the constructed command before the nested telemetry process can enforce its mode.
...
- `ob-deploy` snapshot `4ef23088...`: `_cmd.py` `ObdCommand.init_home`, `_init_log`, `do_command`, and `ClusterMirrorCommand.background_telemetry_task`; `ssh.py` `LocalClient.execute_command_background`.
