...
- The wizard can enable persistent systemd auto-start. In the inspected implementation, that branch creates a privileged unit and can broaden permissions on the deployment `tmp` directory. Keep it disabled unless the user requests persistent startup; then show the unit, permission, credential, and rollback effects and obtain host-change authorization.
