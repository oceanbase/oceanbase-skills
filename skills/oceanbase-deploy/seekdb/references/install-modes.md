...
- The integrated path reconnects through a shared precheck whose failure message includes the literal SSH password. Use this path only with an empty password over a proven local, key, or agent-based SSH route. If password authentication is required, stop and use a separately reviewed configuration-file or fixed-version workflow. If a failure occurs after a password was supplied, protect the terminal/trace as credential-bearing evidence and rotate that credential.
- The wizard can enable persistent systemd auto-start. In the inspected implementation, that branch constructs shell text from raw paths and a sudo password, logs the command, creates a privileged unit, and recursively grants other users `rwx` on the deployment `tmp` directory. Keep auto-start **disabled**. Enable it only in a later implementation proved to use safe argument/credential handling, bounded permissions, and a reviewed unit rollback.
- The inspected path text-interpolates the SSH password and a copied primary root password into generated YAML. Keep secrets out of chat and reusable commands, and do not use this path with a non-empty value unless the installed implementation proves YAML-safe serialization and protected storage for the actual value. Do not weaken or silently replace a credential to satisfy the wizard; use a fixed workflow when the round trip cannot be proved.
...
Use this staged path for scripted or CI deployment only after installed help, schema, artifacts, paths, credentials, and start behavior have been reviewed. Keep `enable_auto_start` false under the inspected implementation. Preserve deploy and start as separate authorization and acceptance stages; do not start after a partial deploy.
...
3. Read `home_path` through the authenticated SQL/OBShell metadata path before invoking takeover. Require exactly one `ocs.all_agent` record for the intended host/port: the inspected query filters only by MySQL port and takes the first row, so a multi-record result can select the wrong home. Require a canonical absolute path matching the conservative lexical allowlist `^/[A-Za-z0-9._/-]+$`, with no `.` or `..` path component, and prove it belongs to the intended process. Otherwise stop: the workflow interpolates the discovered path into remote shell text. Require the SSH identity to equal the actual path/process owner; do not accept its warning-and-continue behavior.
...
- `ob-deploy` snapshot `4ef23088...`: `core.py` SeekDB install/takeover methods; interactive, host-tool, SeekDB HA, auto-start, and takeover plugins.
