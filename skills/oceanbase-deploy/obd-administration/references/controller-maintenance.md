...
1. Record the controller, user, exact executable, RPM owner, OBD version/build, runtime, installation prefix, `OBD_HOME`, active OBD tasks, and every registered deployment.
...
6. Wait until no lifecycle command, repository mutation, tool installation, or test orchestration is active. Show the controller outage or command-availability impact and the complete rollback artifact before authorization.
...
Repository enable/disable changes and local artifact import are separate controller mutations and must follow the repository workflow.
...
- Official OBD V4.6.0 OBD Upgrade Guide for RPM-managed controllers.
