...
Before resolving a package, repository, or installation method, apply the shared [highest-priority OBD privilege gate](../../references/operation-contract.md#highest-priority-obd-privilege-gate). The controller login user must be root or pass `sudo -n true`. Otherwise stop and report that the current Skill version does not support using OBD with that login user; ask the user to switch users or configure usable sudo. Do not edit sudoers or substitute a user-local, all-in-one, source, or manually extracted OBD installation.
...
| Ownership and privilege | Intended controller user, package/prefix/profile owners, required package-manager or installer privilege, umask, and shared automation | The login user is non-root and cannot run usable non-interactive sudo, or the method would overwrite another owner |
...
After substituting a reviewed absolute path, install the exact `ob-deploy` RPM through the target OS package manager with its ordinary non-interactive acceptance option. A root login runs `yum install -y <absolute_reviewed_ob_deploy_rpm>` directly; a non-root login that passed the privilege gate runs `sudo yum install -y <absolute_reviewed_ob_deploy_rpm>`. A proved enabled repository may instead use `yum install -y ob-deploy` as root or `sudo yum install -y ob-deploy` as the admitted non-root user, although the default clean-controller bootstrap remains direct download of the exact RPM. When the complete dependency closure is already proved and unrelated enabled repositories are the only blocker, use the corresponding root or sudo-prefixed `yum --disablerepo='*' install -y <absolute_reviewed_ob_deploy_rpm> <absolute_reviewed_dependency_rpm> ...` form, or its installed-help equivalent. Do not ask the user to approve these reviewed first-install commands. Package-manager `-y` is a scoped first-install exception, not permission to enable controller-wide automatic confirmation or to accept OBD lifecycle, destructive, replacement, upgrade, or downgrade prompts. Do not use the all-repositories-disabled form with unresolved dependencies, and do not use a glob or remote repository candidate when an exact version was requested.
