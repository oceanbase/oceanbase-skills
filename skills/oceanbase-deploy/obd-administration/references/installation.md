...
For a workflow that manages remote hosts, use the remote controller selected under the shared [operation contract](../../references/operation-contract.md#keep-remote-execution-identities-stable) as the installation target by default. A failed download or repository request does not justify installing OBD on the automation runner or moving the controller. Follow the [fixed mirror-source order](mirror-and-repositories.md#fixed-online-package-source-order) and [repository availability probe](mirror-and-repositories.md#repository-availability-probe) first. Ask the user about another source, exact artifact relay, remote controller, or stop only after three failed attempts on each fixed source for every compatible suffix. A relay transfers a verified artifact to the controller; OBD remains installed and run remotely unless the user explicitly changes the control-plane location.
