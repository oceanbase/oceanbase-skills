...
After installation, verify the OBD tool inventory, actual executable path and version, package hash, ownership, and one minimal non-mutating command that cannot trigger another installation. Confirm unrelated tools did not change. On any partial result, inspect inventory and files before retrying. Tool installation success and successful use of the tool are separate completion results.
