...
Read the exact installed subcommand help. Some builds default omitted host or user arguments to localhost or the current user; never rely on those defaults for a remote or privileged operation. Keep passwords out of command arguments when a key or protected input is available.

## Precheck

Treat a host precheck as read-only only after confirming the installed implementation and selected arguments. Preserve the exact checks, targets, output, and OBD identity. A host-level pass does not prove that a particular deployment's packages, topology, paths, ports, resources, or component configuration are valid.
